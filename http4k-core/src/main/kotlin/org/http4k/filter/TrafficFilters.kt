...
object TrafficFilters {
    ...
    object ServeCachedFrom {
        operator fun invoke(source: Source) = Filter { next -> { source[it] ?: next(it) } }
    }
     ...
     */
    object RecordTo {
        operator fun invoke(sink: Sink) = Filter { next -> { next(it).apply { sink[it] = this } } }
    }
    ...
    object ReplayFrom {
        operator fun invoke(replay: Replay,
            ...
            val ret: suspend (Request) -> Response = {
