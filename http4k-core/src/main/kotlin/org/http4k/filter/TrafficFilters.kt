...
object TrafficFilters {
    ...
    object ServeCachedFrom {
        operator fun invoke(source: Source) = Filter { next -> HttpHandler { source[it] ?: next(it) } }
    }
     ...
     */
    object RecordTo {
        operator fun invoke(sink: Sink) = Filter { next -> HttpHandler { next(it).apply { sink[it] = this } } }
    }
}